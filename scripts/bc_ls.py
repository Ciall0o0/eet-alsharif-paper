# bc_ls.py — label-smoothing BC baseline (R6: compare with soft-label cloning)
# LS_EPS=0.1: target = (1-eps)*onehot + eps/3. Same LB-teacher protocol.
...
SEQ = 64
LS_EPS = float(os.environ.get("LS_EPS", "0.1"))
...
if __name__ == "__main__":
    ...
    OUT_TAG = os.environ.get("OUT_TAG", "bc_ls01")
    ...
    print(f"LS eps={LS_EPS} demos={n_demo} decisions={sum(len(o) for o,_ in demos)}", flush=True)
    ...
    for epoch in range(n_epochs):
        ...
        for obs_arr, act_arr in demos:
            ...
            for s in range(0, n, SEQ):
                ...
                target = (1 - LS_EPS) * eye[act_t] + LS_EPS / 3.0
