...
Science Foundation of China under Grant xxxxxxxx. Code available at:
\texttt{https://github.com/Ciall0o0/eet-alsharif-paper}.}

\markboth
{Song: Zone-Aware GRU--RL Dispatch}
{Song: Zone-Aware GRU--RL Dispatch}
...
proposed agent improves mean per-episode reward 3.0$\times$ over the
best rule baseline (SD-ETA; pooled Welch $t = 4.91$,
...
rules' average waiting time with far less empty travel, and
...
zone head reaches training accuracy 0.58--0.73 against a 0.60
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_architecture_ieee.pdf}
...
$\beta$ (outgoing), $\gamma$ (interfloor) ratios and baseline arrival rates
(1$\times$; training runs at 2$\times$, and the peak evaluation
regime in Section~\ref{sec:results} at 2.5$\times$).}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig_main_results_ieee.pdf}
...
majority-class prior (training distribution), i.e. it recovers the temporal--spatial
...
cross zero and collapse---SD-ETA serves only 831 of 3{,}500
passengers, and at the cost of excessive empty travel that makes its
reward 3$\times$ worse than the learned policy---while the learned
policy remains near its comfort-zone
